...
def parse_takeout_records(payload: object) -> Iterable[VisitRecord]:
    ...
    if isinstance(payload, list):
        ...
        records_raw = (
            typed_payload.get("Browser History")
            or typed_payload.get("BrowserHistory")
            or typed_payload.get("history")
            or typed_payload.get("items")
        )
        if not isinstance(records_raw, list):
            raise ValueError("Unsupported Takeout export structure; expected list of entries.")
        for entry_obj in cast(list[object], records_raw):
            if isinstance(entry_obj, dict):
                entry_dict = cast(dict[str, object], entry_obj)
                records.append(entry_dict)
    else:
        raise ValueError("Takeout export must be a list or dict containing Browser History.")

    for entry in records:
        url = entry.get("url")
        timestamp_raw = (
            entry.get("time_usec") or entry.get("timestamp_usec") or entry.get("timeUsec")
        )
        if timestamp_raw is None:
            timestamp_raw = entry.get("time") or entry.get("timestamp")
...
def load_file(
    ...
    *,
    ...
    records = parse_takeout_records(payload)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Load Google Takeout browser history into SQLite.")
    parser.add_argument("input", type=Path, help="Path to Takeout JSON file (Browser History).")
    parser.add_argument(
